...
correo.enviar = function enviar(email, template, nombre,  id, fecha) {
    ...
    if (template == "confirmacion") {
        ...
        contentHTML = `<p>El día <b>${fecha}</b> te esperamos en el Rocódromo!</p><p> Si deseas modificar o cancelar tu cita, puedes hacerlo con el código <b>${id}</b> en nuestra sección de Reservas.</p><hr>`;
        confirmaURL = `<p>Si deseas confirmar tu cita <a href="http://localhost:8080/reservas/confirmar/${id}">pulsa aquí</a>`
    ...
    let transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            user: 'ampacabezas@gmail.com',
            pass: 'hkxgtcqldzofmftz',
        }
    });
